...
`exports-md` resolves the nearest `node_modules/typescript` from the current working directory, so declaration emit uses the TypeScript version installed by the target project. This TypeScript installation is also used to parse declarations from fetched packages. For `.tsrx` inputs or projects with `@tsrx/typescript-plugin` installed, it uses that workspace's `tsrx-tsc` wrapper and configured TSRX compiler to emit declarations. TSRX support therefore requires TypeScript and the TSRX plugin/compiler to be installed in the workspace containing the source. Relative re-exports to `.tsrx` files are supported when written with the `.tsrx` extension or when TypeScript can resolve the import; extensionless `.tsrx` imports are resolved directly. The generated declarations are rendered, so implementation bodies are omitted. If `target` is omitted from the project config, declaration emit defaults to ES2022; explicit compiler settings are retained.
