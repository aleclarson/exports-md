...
The tool emits declarations in memory, parses the resulting `.d.ts`, and renders Markdown sections from exported declarations and their leading TSDoc comments. Property TSDoc comments stay inside declaration code blocks by default. With `--property-docs list`, property comments are removed from interface and object type code blocks and rendered below the block as a `**Properties**` list. When a symbol query is provided, the output includes requested exports plus local declaration dependencies needed to understand their signatures. With `--sortExports`, same-module export sections are printed by category: functions, classes, constants, remaining non-types, then types. With `--reverseSymbols`, rendered symbol sections are printed in reverse order while heading and reference import/re-export blocks stay in place.
