...
`exports-md` resolves the nearest `node_modules/typescript` from the current working directory, so declaration emit uses the TypeScript version installed by the target project. This TypeScript installation is also used to parse declarations from fetched packages. For `.tsrx` inputs or projects with the TSRX TypeScript plugin installed, it uses that workspace's `tsrx-tsc` wrapper and configured TSRX compiler to emit declarations. If `target` is omitted from the project config, declaration emit defaults to ES2022; explicit compiler settings are retained.
