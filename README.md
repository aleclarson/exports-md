...
When the input is `package.json`, the tool reads the `exports` field and renders each declaration entry point with a separate H1 based on the package name and export subpath, such as `foo` for `.` and `foo/bar` for `./bar`. Export-map entries with `types` targets use those targets. Wildcard targets are expanded against the package files and rendered with their concrete export subpaths. Entries without `types` targets use string `.js` or `.mjs` targets rewritten to `.d.ts`; non-JavaScript/TypeScript entries such as `./package.json` are skipped. With `--outDir`, each entry point is written as a `.md` file under the output directory, preserving the entry point folder structure relative to their shared common root.
